...
from wagtail.core.blocks import ChooserBlock, ModelChoiceBlock
...
class CodeStatisticLanguageBlock(blocks.StructBlock):
    ...
    @staticmethod
    def generate_block(
        language_name: "Unkown", color: "#ffffff", insertions: "0", deletions: "0",
    ):
        return {
            "type": "language",
            "value": {
...
class CodeStatisticBlock(blocks.StructBlock):
    ...
    def generate_block(
        insertions="0",
        deletions="0",
        datetime=timetool.datetime.now(tz=timetool.timezone.utc).isoformat(),
    ):
        return {
            "type": "history",
            "value": {
...
class FeedCommitFileBlock(blocks.StructBlock):
    ...
    @staticmethod
    def generate_block(
        insertions="0", deletions="0", path=".", raw_changes="",
    ):
        return {
            "type": "file",
            "value": {
...
class _S_ProjectBlock(blocks.StructBlock):
    ...
    #     label="Brand",
    #     target_model="ContributionFile",  # or model
    #     required=False,
    #     to_field_name="slug",
    # )
    test = ModelChoiceBlock(target_model=ContributionFeed)
    # feed = ParentalKey(
    #     "ContributionFeed", on_delete=models.CASCADE, related_name="Test1234"
    # )
...
class OpsScpagePage(Page):
    ...
    def generateContent(self):
        ...
        tmp_projects_section = []
        for project in data:
            print(project["maintainer_email"])

            tmp_project_feed_data = []
            for event in project["events"]:
                tmp_project_feed_data.append(
                    FeedCommitBlock.generate_block(
                        id=event["id"],
                        type="commit",
            ...
            feed = [FeedBlock.generate_block(feed_data=tmp_project_feed_data)]

            tmp_projects_section += [
                _S_ProjectBlock.generate_block(
                    name=project["name"],
                    url=project["url"],
                    description=project["description"],
                    maintainer_name=project["maintainer_name"],
                    maintainer_username=project["maintainer_username"],
                    maintainer_email=project["maintainer_email"],
                    feed=feed,
                )
            ]

            print(tmp_projects_section)

        self.projects_section = json.dumps(tmp_projects_section)
        self.save()
