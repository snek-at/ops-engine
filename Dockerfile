...
ENV UWSGI_WSGI_FILE=esite/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/esite/media/"
...
RUN mkdir -p /code/esite/media/images && chown -R 1000:2000 /code/esite/media

# mark the destination for images as a volume
VOLUME ["/code/esite/media/images/"]
